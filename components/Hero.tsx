      ...
      <div className="md:w-[67%] mx-auto">
        ...
        <div className="flex flex-col gap-4 mb-6">
          {secondaryArticles.map((article, index) => (
            <div key={index}>
              <h3 className="text-lg font-semibold mb-1">{article?.title || "No title"}</h3>
              <p className="text-gray-700 mb-1">{article?.description || "No description"}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {randomArticles.map((article, index) => (
            <div key={index} className="p-4">
              ...
              <h3 className="text-base font-semibold mb-1">{article?.title || "No title"}</h3>
              <a href={article?.url || "#"} target="_blank" rel="noopener noreferrer" className="text-blue-500">
                Read more
              </a>
            </div>
